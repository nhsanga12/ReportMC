<?php
	...
	# Lấy ID;
		...
		if(!empty($id['detail'])){
			...
			$title	=	articles_detail();
		}
		if (!empty($title)) {
			$id['title'] = $title[0]['title'];
			$id['description'] = sys_cut(html2text($title[0]['contents']), 400);
			$id['image'] = $title[0]['picture'];
		}
		if (empty($id['title'])) 
	...
	if (isset($_GET['language']) && !is_null($_GET['language']))
