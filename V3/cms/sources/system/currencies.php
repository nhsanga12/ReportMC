<?php
	if ($id['option'] == "main") {
		$sql = "SELECT id, currency_title AS title FROM ".$config['db_prefix']."_currencies ";
		$sql.= "WHERE currency_title LIKE '%".addslashes($id['search'])."%' ORDER BY bydate DESC ";
	...
	if ($id['option'] == "add") {
		if (isset($_POST['submit'])) {
			...
			if ($msg == '') {
				$rs['currency_title']	=	$_POST['add_title'];
				$rs['currency_value']	=	$_POST['add_key_value'];
				$rs['bydate']	=	time();
				$rs['lastdate']	=	time();
				$newid = sql_add('currencies',$rs);
	...
	if ($id['option'] == "edit") {
		...
		$rs['currency_value']	=	$_POST['add_key_value'];;
		$rs['lastdate']	=	time();
		sql_update('currencies',$rs,$rsk); 
		$msg = 'Đã cập nhật thành công <strong>'.$_POST['add_title'].'</strong>';
		}
		$sql = "SELECT id,currency_title AS title, currency_value FROM ".$config['db_prefix']."_currencies ";
